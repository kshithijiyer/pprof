...
func findObjdump(paths []string) (string, bool, bool) {
	...
	if objdump, objdumpFound := findExe("llvm-objdump", paths); objdumpFound == true {
		allObjdump = append(allObjdump, objdump)
	}

	if objdump, objdumpFound := findExe("llvm-objdump-9", paths); objdumpFound == true {
		allObjdump = append(allObjdump, objdump)
	}

	buObjdumpFound := false
	var buObjdump string
	if objdump, objdumpFound := findExe("objdump", paths); objdumpFound == true {
	...
	if !buObjdumpFound {
		if objdump, objdumpFound := findExe("gobjdump", paths); objdumpFound == true {
			buObjdump = objdump
			buObjdumpFound = true
		}
	}
